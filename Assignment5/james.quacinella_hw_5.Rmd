...
plot(ukcars.addDamped, plot.conf=FALSE, fcol="white", type="o")
lines(fitted(ukcars.addDamped), col="blue", type="o")
legend(1980, 450, c("Data","Fit (Damped)"), lty=c(1,1), lwd=c(2.5, 2.5), col=c("black", "blue"))
```
...
# Show the fitted value
plot(ukcars.linear, plot.conf=FALSE, fcol="white", type="o")
...
# Show the fitted value
plot(ukcars.ets, plot.conf=FALSE, fcol="white", type="o")
...
```{r cached=TRUE}
rbind(accuracy(ukcars.ets), accuracy(ukcars.linear), accuracy(ukcars.addDamped))
```
...
legend("bottomright", inset=c(0,0), legend=c("Holt-Winters' seasonal mult", "ETS", "ETS Additive", "Seasonal Naive", "STL then ETS"), lty=c(1,1,1,1), lwd=c(2.5, 2.5, 2.5, 2.5), col=c("purple","blue","red","green", "#fdae61"))
